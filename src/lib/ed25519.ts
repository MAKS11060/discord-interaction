...
/**
 * Imports a public key from a `hexadecimal` string and returns a {@linkcode CryptoKey} object.
 *
 * @param {string} publicKey - The public key in hexadecimal format.
 * @returns {Promise<CryptoKey>} A promise that resolves to a {@linkcode CryptoKey} object representing the imported public key.
 */
 ...
 * and verifies the signature using the provided {@linkcode CryptoKey} object. If the signature is invalid,
 * the function returns a {@linkcode Response} object with a 401 status code and an error message.
 *
 ...
 * @returns {Promise<Response | null>} A promise that resolves to a {@linkcode Response} object with an error message if the signature is invalid, or null if the signature is valid.
 */
export const verifyRequestSignature = async (
  ...
  const valid = await crypto.subtle.verify(
    ...
    key,
    decodeHex(signature),
    concat([encoder.encode(timestamp), await request.clone().bytes()]) // (timestamp + body)
  )
  if (!valid) {
    return Response.json({error: 'Bad request signature'}, {status: 401})
  }
...
export const signRequest = async (req: Request, key: CryptoKey): Promise<Request> => {
  ...
  const signature = await crypto.subtle.sign(
    'Ed25519',
    key,
    concat([encoder.encode(timestamp), await req.clone().bytes()]) // (timestamp + body)
  )
